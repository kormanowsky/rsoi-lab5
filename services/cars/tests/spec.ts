import { CarsLogic, Car } from '../src/logic';
import { mockCar, MockCarsStorage } from './mocks';

describe('CarsLogic', () => {
    let logic: CarsLogic;

    beforeEach(() => {
        logic = new CarsLogic(new MockCarsStorage([mockCar]));
    });
        ...
        for(const showAll of [true, false]) {
            describe(`с showAll = ${showAll}`, () => {
                test('без пагинации', async () => {
                    const cars = await logic.getMany({showAll, page: 1, size: Infinity});
            
                    expect(cars).toHaveLength(showAll ? 6 : 4);
